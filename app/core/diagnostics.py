...
def create_demo_project():
    ...
    try:
        ...
        blender_py_expr = (
            ...
            "# --- SCENE 1: Salon ---\n"
            "clear_mesh_objects()\n"
            "# Create basic ground and objects\n"
            "bpy.ops.mesh.primitive_plane_add(size=10, location=(0, 0, 0))\n"
            "bpy.ops.mesh.primitive_monkey_add(size=2, location=(0, 0, 1))\n"
            "bpy.ops.mesh.primitive_uv_sphere_add(radius=1, location=(-3, 2, 1))\n"
            "bpy.ops.mesh.primitive_cone_add(radius1=0.8, depth=2, location=(3, -2, 1))\n"
            "\n"
            "# Setup cameras\n"
            ...
            "\n"
            "cam_a = bpy.data.cameras.new('Camara A')\n"
            "cam_a_obj = bpy.data.objects.new('Camara A', cam_a)\n"
            "cam_a_obj.location = (0, -7, 3)\n"
            "cam_a_obj.rotation_euler = (1.2, 0, 0)\n"
            "bpy.context.scene.collection.objects.link(cam_a_obj)\n"
            "bpy.context.scene.camera = cam_a_obj  # Make active\n"
            "\n"
            "cam_b = bpy.data.cameras.new('Camara B')\n"
            "cam_b_obj = bpy.data.objects.new('Camara B', cam_b)\n"
            "cam_b_obj.location = (-6, 5, 2.5)\n"
            "cam_b_obj.rotation_euler = (1.1, 0, -0.8)\n"
            "bpy.context.scene.collection.objects.link(cam_b_obj)\n"
            "\n"
            "cam_c = bpy.data.cameras.new('Camara C')\n"
            "cam_c_obj = bpy.data.objects.new('Camara C', cam_c)\n"
            "cam_c_obj.location = (6, -5, 2.5)\n"
            "cam_c_obj.rotation_euler = (1.1, 0, 0.8)\n"
            "bpy.context.scene.collection.objects.link(cam_c_obj)\n"
            "\n"
            "# Configure frame start/end to 1 (Still)\n"
            "bpy.context.scene.frame_start = 1\n"
            "bpy.context.scene.frame_end = 1\n"
            "\n"
            "# Save Salon blend\n"
            "bpy.ops.wm.save_as_mainfile(filepath=r'" + os.path.join(blend_dir, "(0000)_Escena_Salon.blend") + "')\n"
            "\n"
            "# Setup Salon Montage\n"
            ...
            "bpy.ops.wm.save_as_mainfile(filepath=r'" + os.path.join(blend_dir, "(0000)_Escena_Salon_montaje.blend") + "')\n"
            "\n"
            "# --- SCENE 2: Exterior (Animation) ---\n"
            "bpy.ops.wm.read_factory_settings(use_empty=True)\n"
            "bpy.ops.mesh.primitive_plane_add(size=20, location=(0, 0, 0))\n"
            "bpy.ops.mesh.primitive_torus_add(location=(0, 0, 0.5))\n"
            "bpy.ops.mesh.primitive_monkey_add(size=1.5, location=(0, 0, 2))\n"
            "\n"
            "cam_terr = bpy.data.cameras.new('Camara Terraza')\n"
            "cam_terr_obj = bpy.data.objects.new('Camara Terraza', cam_terr)\n"
            "cam_terr_obj.location = (0, -10, 4)\n"
            "cam_terr_obj.rotation_euler = (1.2, 0, 0)\n"
            "bpy.context.scene.collection.objects.link(cam_terr_obj)\n"
            "bpy.context.scene.camera = cam_terr_obj\n"
            "\n"
            "cam_jard = bpy.data.cameras.new('Camara Jardin')\n"
            "cam_jard_obj = bpy.data.objects.new('Camara Jardin', cam_jard)\n"
            "cam_jard_obj.location = (-8, -8, 3)\n"
            "cam_jard_obj.rotation_euler = (1.3, 0, -0.6)\n"
            "bpy.context.scene.collection.objects.link(cam_jard_obj)\n"
            "\n"
            ...
            "\n"
            "# Save Exterior blend\n"
            "bpy.ops.wm.save_as_mainfile(filepath=r'" + os.path.join(blend_dir, "(0000)_Escena_Exterior.blend") + "')\n"
            "\n"
            "# Setup Exterior Montage\n"
            "bpy.context.scene.use_nodes = True\n"
            "tree = bpy.context.scene.node_tree\n"
            "for n in tree.nodes: tree.nodes.remove(n)\n"
            "img_node = tree.nodes.new('CompositorNodeImage')\n"
            ...
            "comp_node = tree.nodes.new('CompositorNodeComposite')\n"
            "tree.links.new(img_node.outputs['Image'], comp_node.inputs['Image'])\n"
            "bpy.ops.wm.save_as_mainfile(filepath=r'" + os.path.join(blend_dir, "(0000)_Escena_Exterior_montaje.blend") + "')\n"
        )
